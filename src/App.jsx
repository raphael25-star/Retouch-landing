...
function DashboardPage({ user, navigate, onLogout, refreshUser, sessionChecked }) {
  ...
  const tools = [
    // ─── TEMPLATES TENDANCES (en premier) ───
    {
      ...
      expertPromptPrefix: "Take the watch from the second image and integrate it onto the wrist in the first image. Keep the first image exactly the same. Do not recreate the scene, the hand, the background, the desk, or the lighting. Only edit the watch integration. The watch must be proportionally adapted to the real wrist size and must follow the natural wrist angle and curvature. The bracelet must wrap around the wrist naturally, with correct perspective and slight skin contact. Match the first photo perfectly: same lighting, same color temperature, same sharpness, same camera quality, same noise and compression, same shadows. Add realistic contact shadows under the bracelet and around the watch case. Slightly soften the watch details so it does not look pasted, rendered, or too perfect. Preserve the watch identity, color, dial, hands, bracelet style, and materials, but make it look like it was actually photographed in this scene. Avoid: product render look, overly sharp watch, studio lighting, perfect reflections, floating effect, changing the wrist or background. Final result must look like a genuine casual iPhone photo, not an AI-generated product placement. User additional instructions: ",      name: "Serpent Prank",
