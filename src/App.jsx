...
function DashboardPage({ user, navigate, onLogout, refreshUser, sessionChecked }) {
  ...
  const tools = [
    // ─── TEMPLATES TENDANCES (en premier) ───
    {
      ...
      expertPromptPrefix: "Place the watch from the second image on the wrist shown in the first image, as if the person is naturally wearing it. The watch bracelet must wrap around and behind the wrist - parts of the bracelet should be visible disappearing behind the arm and reappearing on the other side, showing that it goes ALL THE WAY around the wrist (3D wrap effect). The watch face sits on top of the wrist. The metal bracelet curves and follows the natural shape of the wrist - it is NOT a flat 2D sticker on the skin. Show realistic contact shadows under the watch where the bracelet touches the skin, and slight pressure on the skin around the bracelet links. Adapt the watch size proportionally to the wrist (a watch case is typically 40-44mm, the bracelet covers about half the wrist circumference visible from front). Preserve EXACTLY the watch details from the source image (dial, hands, bezel, brand, color, materials). Match the lighting direction, color temperature, and skin tone of the first photo. Photorealistic smartphone photo style with natural grain. The result should look like a candid photo of someone wearing the watch, not a product placement composite. User additional instructions: ",      promptTemplate: ""
    },
    {
      name: "Serpent Prank",
